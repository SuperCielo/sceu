...
object OrderValidator {
  ...
  private def validateBalance(order: Order, tradableBalance: Option[AssetId] => Long): ValidationResult = {
    ...
    val requiredForOrder = Monoid.combine(
      Map(lo.spentAsset -> lo.getRawSpendAmount),
      Map(lo.feeAsset   -> lo.fee)
    )
    ...
    Either.cond(
      Monoid.combine(available, requiredForOrder.mapValues(-_)).forall(_._2 >= 0),
      order,
      s"Not enough tradable balance. Order requires $requiredForOrder, but only $available is available"
    )
  }
